...
- **User identity model.** Add a user-email field to the form (default `john@johnlandry.com`); replace hardcoded `user_id = 1` in `save_recipe`. Eventual upstream: Ghost. Every recipe is owned by a user; duplicate source URLs across users are intentionally fine — each gets their own customizable row.
- **Token capture for billing.** Thread `response.usage.{prompt_tokens, completion_tokens, total_tokens}` through `markdown_to_recipe`, `image_to_markdown`, and `enrich_recipe`. Surface as `_usage` on each extract endpoint response (parallel to `_timings`/`_prompt`).
- **General ledger / transactions.** One row per chargeable LLM call: `transaction_id, user_id, recipe_id, timestamp, operation, model, prompt_tokens, completion_tokens, total_tokens, estimated_cost_usd, subscription_tier_at_time`. Monthly aggregation queries → per-user usage and invoicing. Subscription tier model (hard cap / soft cap / overage) still TBD.
