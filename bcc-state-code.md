...
- **NOT YET — selection half (task #15):** winner SELECTION uses `rank_by_blend` (Python, build_query_batch), which lacks the remap → paid sites rank fair in the cohort PANEL but still aren't SELECTED winners. Must apply the same remap to entries' PA before the OU fit + rank_by_blend (store RAW pa in fit_data_points; score_data_points re-applies → consistent). Same dual-ranker split as the Cajun finding.
- Also future: authenticated Playwright fetch to INGEST gated recipes (CI/Milk Street) into the corpus at all; sitemap discovery for the harvester.

### Next
- **Paid-PA SELECTION half** (task #15): remap in `rank_by_blend` so paid recipes actually become winners, not just rank well in the panel.
