...
- **v2 follow-ups (planned):** sub-step the mise itself (check-to-advance per cluster); **verbosity preference** read at cook-page gen (verbose↔concise → expertise fade); optional on-screen per-sub-step highlight (the `screen` fragments are authored but the renderer still shows the whole step); follow-up research run on interruption/place-keeping + applied checklists/TWI.
