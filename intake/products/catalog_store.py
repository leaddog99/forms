...
def _fill_missing(existing: dict, incoming: dict) -> None:
    ...
    for f in ("image_url", "category", "product_class"):
...
def save_product(conn: sqlite3.Connection, product: dict, *,
    ...
    if target:
        ...
        if row:
            ...
            blob = vec_to_bytes(embed_text(compose_product_text(existing))) if (embed and _EMBED_OK) else None
            if blob is not None:
    ...
    pid = str(uuid.uuid4())
    blob = vec_to_bytes(embed_text(compose_product_text(p))) if (embed and _EMBED_OK) else None
    conn.execute(
